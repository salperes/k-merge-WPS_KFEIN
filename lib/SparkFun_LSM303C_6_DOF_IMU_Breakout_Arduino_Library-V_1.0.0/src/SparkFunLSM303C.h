...
class LSM303C : public SparkFunIMU
{
  public:
    ...
    ~LSM303C()  =  default;
    status_t begin(int GAIN);
    // Begin contains hardware specific code (Pro Mini)
    status_t begin(InterfaceMode_t, MAG_DO_t, MAG_FS_t, MAG_BDU_t, MAG_OMXY_t,
       MAG_OMZ_t, MAG_MD_t, ACC_FS_t, ACC_BDU_t, uint8_t, ACC_ODR_t, MAG_TEMP_EN_t);
    float readAccelX(void);
    float readAccelY(void);
    float readAccelZ(void);
    int16_t   readMagX(void);
    int16_t   readMagY(void);
    int16_t   readMagZ(void);
	uint16_t  readTempValue(void);
  ...
  protected:
    ...
    int16_t X,Y,Z;
    AxesRaw_t accelData = {X, Y, Z};
    AxesRaw_t   magData = {X, Y, Z};
