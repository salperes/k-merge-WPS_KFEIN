...
// Public methods
status_t LSM303C::begin(int GAIN)
{
  debug_println(EMPTY);
  return
...
status_t LSM303C::begin(InterfaceMode_t im, MAG_DO_t modr, MAG_FS_t mfs,
    ...
    ACC_FS_t afs, ACC_BDU_t abu, uint8_t aea, ACC_ODR_t aodr, MAG_TEMP_EN_t temp)
{
  uint8_t successes = 0;
  // Select I2C or SPI
  interfaceMode = im;
  ...
  //TEMP SENSORU AC
  successes += MAG_TemperatureEN(temp);

  return (successes == IMU_SUCCESS) ? IMU_SUCCESS : IMU_HW_ERROR;
}

int16_t LSM303C::readMagX()
{
  return readMag(xAxis);
}

int16_t LSM303C::readMagY()
{
  return readMag(yAxis);
}

int16_t LSM303C::readMagZ()
{
...
uint16_t LSM303C::readTempValue()
{
  uint8_t valueL;
  uint8_t valueH;
  ...
  temperature = ((uint16_t)(valueH << 8) | valueL );
